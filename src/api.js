...
export const getFamily = (familyId, user) =>
  axios({
    ...
    data: JSON.stringify({
      query:
        'query familyById($id: Long) { familyById(id: $id) {familyId name code organization { name } country{country} familyMemberDTOList{firstParticipant email phoneNumber phoneCode}  }}',
      variables: {
        id: familyId
      }
    })
